...
async function refreshProjectPorts(state, project) {
  const otherProjects = state.projects.filter(item => item !== project);
  const used = new Set(otherProjects.flatMap(item => [item.httpPort, item.httpsPort]));
  const currentPortsAreFree = Number.isInteger(project.httpPort)
    && Number.isInteger(project.httpsPort)
    && project.httpPort > 0
    && project.httpsPort > 0
    && project.httpPort <= 65535
    && project.httpsPort <= 65535
    && project.httpPort !== project.httpsPort
    && !used.has(project.httpPort)
    && !used.has(project.httpsPort)
    && await isPortFree(project.httpPort)
    && await isPortFree(project.httpsPort);
  if (currentPortsAreFree) return false;

  Object.assign(project, await allocatePorts({ ...state, projects: [...otherProjects, project] }));
...
function caddyfile(state) {
  const blocks = state.projects.map(project => {
    if (project.id === 'timeline') {
      return `http://timeline.rb {\n\treverse_proxy 127.0.0.1:${project.httpPort}\n}\n\nhttp://100.75.92.26 {\n\treverse_proxy 127.0.0.1:${project.httpPort}\n}\n\ntimeline.rb:8443, 100.75.92.26:8443 {\n\ttls /home/remotebox/projects/twitterscrapper/.tls/timeline.crt /home/remotebox/projects/twitterscrapper/.tls/timeline.key\n\treverse_proxy 127.0.0.1:${project.httpPort}\n}`;
    }
    const scheme = project.https ? '' : 'http://';
    const tls = project.https ? '\n\ttls internal' : '';
    const address = project.https ? `${project.url}:8443` : `${scheme}${project.url}`;
    const isHyundai = project.url === 'hyundai.pb';
    const wildcard = isHyundai ? `, *.${project.url}${project.https ? ':8443' : ''}` : '';
    const upstream = isHyundai
      ? `https://127.0.0.1:${project.httpsPort} {\n\t\theader_up Host {http.request.host}\n\t\ttransport http {\n\t\t\ttls_insecure_skip_verify\n\t\t}\n\t}`
      : `127.0.0.1:${project.httpPort}`;
    const redirect = project.https
      ? `http://${project.url}${isHyundai ? `, http://*.${project.url}` : ''} {\n\tredir https://{host}{uri} permanent\n}\n\n`
      : '';
    const local = `${redirect}${address}${wildcard} {${tls}\n\treverse_proxy ${upstream}\n}`;
    return local;
  }).join('\n\n');
  const tail = `remotebox-1.tail469081.ts.net:8443 {\n\ttls /home/remotebox/projects/twitterscrapper/.tls/remotebox-1.crt /home/remotebox/projects/twitterscrapper/.tls/remotebox-1.key\n\treverse_proxy 127.0.0.1:3773\n}`;
...
async function startManagedProject(state, project, foreground = false) {
  ...
  await refreshProjectPorts(state, project);
  let child;
  try {
